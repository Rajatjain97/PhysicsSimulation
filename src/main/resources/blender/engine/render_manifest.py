...
reproducible later: it names the template and the contract version that produced the image, and
carries the render id Java assigned, so a file on disk can always be traced back to a run.
"""
...
def write_manifest(image_path: str, render_id: str, contract: SceneContract, resolution: str) -> str:
