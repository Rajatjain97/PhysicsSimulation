 ...
 *
 * <p>Deliberately separate from the application output folders: {@code output/videos} and
 * {@code output/thumbnails} hold deliverables the user cares about, while everything here is the
 * render engine's working area and can be deleted without losing anything.
 *
 * <p>It is a narrow view over {@link WorkspaceLayout} so render code can say
 * {@code renderWorkspace.scripts()} instead of remembering which enum constant to look up.
 *
 * @param scripts   Blender Python scripts, installed from the classpath on startup
 * @param templates reusable .blend scene templates
 ...
 */
public record RenderWorkspace(Path scripts, Path templates, Path renders, Path cache) {
    ...
    /** All four directories, in declaration order. */
    public List<Path> directories() {
        return List.of(scripts, templates, renders, cache);
    }
}
